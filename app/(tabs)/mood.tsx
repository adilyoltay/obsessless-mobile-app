...
import { getVAColorFromScores, lighten } from '@/utils/colorUtils';
...
import { toUserLocalDate, formatDateInUserTimezone } from '@/utils/timezoneUtils';
...
function MoodScreen() {
  ...
  // Helpers: consistency for metrics & notes
  const isAutoGeneratedNote = (text: string | undefined) => {
    if (!text) return false;
    const t = String(text).trim();
    // Matches: "Duygu: <label>, Enerji: <label>" exactly (auto-generated default)
    return /^Duygu:\s*[^,]+,\s*Enerji:\s*[^,]+$/i.test(t);
  };

  // Header date label per selected time range
  const dateLabel = React.useMemo(() => {
    const today = toUserLocalDate(new Date());
    if (selectedTimeRange === 'today') {
      return today.toLocaleDateString('tr-TR', { month: 'long', day: 'numeric', year: 'numeric' });
    }
    const end = today;
    const ms = 24 * 60 * 60 * 1000;
    const daysBack = selectedTimeRange === 'week' ? 6 : 29; // inclusive range
    const start = new Date(end.getTime() - daysBack * ms);
    const startStr = formatDateInUserTimezone(start, 'medium');
    const endStr = formatDateInUserTimezone(end, 'medium');
    return selectedTimeRange === 'week'
      ? `Son 7 gün: ${startStr} – ${endStr}`
      : `Son 30 gün: ${startStr} – ${endStr}`;
  }, [selectedTimeRange]);

  // Map 1..10 energy to textual label (aligned with VA step labels)
  const energyLabelFrom10 = (v?: number) => {
    const n = Number(v || 0);
    if (!Number.isFinite(n) || n <= 0) return '—';
    if (n <= 2) return 'Çok yorgun';
    if (n <= 4) return 'Yorgun';
    if (n <= 6) return 'Normal';
    if (n <= 8) return 'Enerjik';
    return 'Çok enerjik';
  };

  // Map 1..10 anxiety to textual label
  const anxietyLabelFrom10 = (v?: number) => {
    const n = Number(v || 0);
    if (!Number.isFinite(n) || n <= 0) return '—';
    if (n <= 3) return 'Düşük';
    if (n <= 6) return 'Orta';
    if (n <= 8) return 'Yüksek';
    return 'Çok yüksek';
  };
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const color = mode === 'static' ? '#10B981' : getVAColorFromScores(score);
    ...
    <ScreenLayout>
      ...
      >
        ...
        <View style={styles.listSection}>
          ...
          {filteredEntries.length === 0 ? (
            <View style={styles.emptyState}>
              ...
              <Text style={styles.emptySubtext}>
                Today sayfasından "Mood Check‑in" butonuna tıklayarak mood kaydı oluşturabilirsiniz
              </Text>
            </View>
          ) : (
            <View style={styles.recordingsContainer}>
              {filteredEntries.map((entry) => {
                const moodColor = getVAColorFromScores(entry.mood_score || 0, entry.energy_level);
                const softBg = lighten(moodColor, 0.92);
                  ...
                  <View 
                    ...
                    style={[
                      ...
                      { 
                        ...
                        borderColor: lighten(moodColor, 0.8),
                        borderWidth: 2,
                      }
                    ]}
                  >
                    <View style={styles.recordingContent}>
                      ...
                      {entry.notes && !isAutoGeneratedNote(entry.notes) && (
                      ...
                      <View style={styles.recordingMetrics}>
                        <View style={styles.metricItem}>
                          ...
                          <Text style={styles.metricText}>Enerji: {energyLabelFrom10(entry.energy_level)}</Text>
                        </View>
                        <View style={styles.metricItem}>
                          <MaterialCommunityIcons name="alert-circle-outline" size={16} color="#6B7280" />
                          <Text style={styles.metricText}>Kaygı: {anxietyLabelFrom10(entry.anxiety_level)}</Text>
                        </View>
                      </View>
                      {/* Actions moved to header for better alignment */}
                    </View>
                  </View>
                );
              })}
          ...
          {/* Load More Button (content footer, shown when more entries exist) */}
